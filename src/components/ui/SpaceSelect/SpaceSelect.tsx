...
const SpaceSelect = ({ space, setSpace, className, ...props }: SpaceSelectProps & ComponentPropsWithoutRef<'button'>) => {

    const { spaces } = useDataContext()
    return (
        <Listbox as='div' className={styles.spaceSelect} value={space} onChange={setSpace} >
            {space && <>
                <Listbox.Button className={`${styles.selectButton} ${className}`} {...props}>
                    {space.name}
                    <img src={unfold} alt="" />
                </Listbox.Button>
